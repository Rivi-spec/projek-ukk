...
class Products extends CI_Controller
{
    ...
    public function add_product()
    {
        ...
        $this->form_validation->set_rules('price', 'Harga produk', 'trim|required|numeric|greater_than_equal_to[0]');
        $this->form_validation->set_rules('stock_s', 'Stok S', 'required|numeric');
        $this->form_validation->set_rules('stock_m', 'Stok M', 'required|numeric');
        $this->form_validation->set_rules('stock_l', 'Stok L', 'required|numeric');
        $this->form_validation->set_rules('stock_xl', 'Stok XL', 'required|numeric');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // Menghitung stok total (penjumlahan stok ukuran)
            $total_stock = $stock_s + $stock_m + $stock_l + $stock_xl;
        
            // Debugging: Periksa nilai total stok
            echo 'Total Stock: ' . $total_stock; // Cek apakah total stok dihitung dengan benar
            ...
            // Buat SKU produk berdasarkan nama, kategori, harga, dan total stok
            $sku = create_product_sku($name, $category_name, $price, $total_stock);
        
            // Menyusun array data produk yang akan disimpan
            $product = [
                ...
                'stock_l' => $stock_l,
                'stock_xl' => $stock_xl,
                'stock' => $total_stock,  // Simpan stok total
                'product_unit' => $unit,
                'picture_name' => $file_name,
                'add_date' => $date
            ];
        
            // Debugging: Cek nilai array produk
            var_dump($product); // Cek apakah data yang dikirim sudah benar
            ...
            // Redirect kembali ke halaman tambah produk
            redirect('admin/products/add_new_product');
        }
    }
    ...
    public function edit_product()
    {
        ...
        $this->form_validation->set_rules('name', 'Nama produk', 'trim|required|min_length[4]|max_length[255]');
        $this->form_validation->set_rules('price', 'Harga produk', 'trim|required|numeric|greater_than_equal_to[0]', array(
            'greater_than_equal_to' => 'Harga tidak boleh kurang dari 0 atau negatif.'
        ));
    ...
    public function product_api()
    {
        ...
        switch ($action) {
            ...
            case 'reduce_product_stock':
                ...
                try {
                    if (strtolower($size) == 'xl') {
                        $this->product->reduce_stock_auto($id, $quantity);
                    } else {
                        $this->product->reduce_stock($id, $size, $quantity);
                    }
                    $response = array('code' => 200, 'message' => 'Stok berhasil dikurangi');
                } catch (Exception $e) {
